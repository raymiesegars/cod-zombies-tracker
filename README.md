...
**Vercel:** Set `DATABASE_URL` with the Supabase pooler URL (port 6543) and append `?pgbouncer=true&connection_limit=4&pool_timeout=15`. Example:
`postgresql://postgres.PROJECT_REF:PASSWORD@aws-0-us-east-1.pooler.supabase.com:6543/postgres?pgbouncer=true&connection_limit=4&pool_timeout=15`
Use `connection_limit=4` so each instance can serve requests promptly. If you see "Unable to check out connection" or P2024, try `connection_limit=1` and increase Supabase pool size (Database Settings → Connection pooling) to 60+. Set `CI=true` in Vercel build env so the sitemap skips DB during build.
