...
const CATEGORY_ORDER_FOR_RULES = [
  ...
  'ROUND_255_SPEEDRUN', 'ROUND_935_SPEEDRUN', 'ROUND_999_SPEEDRUN', 'EXFIL_SPEEDRUN', 'EXFIL_R21_SPEEDRUN', 'BUILD_EE_SPEEDRUN', 'PACK_A_PUNCH_SPEEDRUN', 'INSTAKILL_ROUND_SPEEDRUN',
      ...
      try {
        ...
        const res = await fetch('/api/challenge-logs', {
          ...
          body: JSON.stringify({
            ...
            ...(map?.slug && hasNoJugSupport(map.slug, map.game?.shortName) && { wawNoJug: challenge?.type === 'NO_JUG' ? true : (form.wawNoJug ?? false) }),
              ...
              <Card variant="bordered">
                <CardContent className="py-4 sm:py-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 min-w-0">
                    ...
                    {isBo3Game(map?.game?.shortName) &&
                      (!Array.from(selectedChallengeIds).some((cid) => map?.challenges.find((c) => c.id === cid)?.type === 'STARTING_ROOM') || (map?.slug && hasFirstRoomGumMachineBo3(map.slug))) && (
                      <>
                        <Select
                          label="GobbleGums"
                          disabled={tournamentLocked}
