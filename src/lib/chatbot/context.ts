...
async function buildWikiContextWithRetrieval(
  ...
  lines.push(
    'Use this to answer questions. ZWR: EE speedrun guides, setup guides, BO3 gobblegum loadouts, terminology — summarize and link zwr.gg/wiki. Skrine: round times (per-round = time for that round; cumulative = total to reach round X). Moon Megas faster than Classics. Terminology (G-spawn), instakill rounds, health scale (HP: 150, 250, 350...), map resets (BO1 vs BO3 differ: Kino 75h vs 105h), Firebase Z trials/quotes, Vanguard transmit (SNN = Shi No Numa), VR-11, zombie health scale by dog round, map reset times, perfect times, point drops, Cold War Firebase Z trials. Point Drops / Raygun / Waffe chunks = BO1 maps (Ascension, Kino, CotD) — NOT WaW Nacht (Nacht has no PaP). Raygun columns = shots needed to clear round (not shot chance). Waffe = shots per horde. Instakill & Health Scale = WaW (Nacht, Verruckt, Shi No Numa, Der Riese). For "raygun shots needed", "waffe shots per horde", "drop chance" — use Point Drops chunk (BO1). For "zombie health dog rounds" — use Instakill chunk (WaW). When the data contains exact numbers, cite them directly. Do not say "I don\'t have that info" if the numbers appear in the Skrine chunks. Prefer our site links for CZT-specific content.'
  );
  lines.push('');
  let len = lines.join('\n').length;
