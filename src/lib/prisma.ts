...
// Default 5 connections per instance so profile, home-stats, messages, notifications, friends, poll-reminder can run in parallel without P2024.
function getDatabaseUrl(): string {
  const url = process.env.DATABASE_URL;
  if (!url) return '';
  const limit = process.env.DATABASE_CONNECTION_LIMIT ? parseInt(process.env.DATABASE_CONNECTION_LIMIT, 10) : 5;
  const connectionLimit = Number.isNaN(limit) || limit < 1 ? 5 : Math.min(limit, 10);
  let out = url
