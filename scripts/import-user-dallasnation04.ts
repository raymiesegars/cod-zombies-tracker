#!/usr/bin/env npx tsx
/**
 * Full transfer for DallasNation04: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmlqzexed0000gqwbwpym1dn9. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/DallasNation04.csv (player column values are display names; source player = DallasNation04)
 * CZT user ID: cmlqzexed0000gqwbwpym1dn9
 *
 * Usage:
 *   pnpm db:import-user-dallasnation04              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-dallasnation04.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmlqzexed0000gqwbwpym1dn9';
const SOURCE_PLAYER_ID = 'DallasNation04';
const CSV_PATH = path.join(root, 'top-178-csv', 'DallasNation04.csv');
...
async function main() {
  ...
  console.log('--- DallasNation04 full transfer ---');
  ...
  console.log('\n--- Done. DallasNation04 transfer complete. ---');
}
