#!/usr/bin/env npx tsx
/**
 * Full transfer for grimoire: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmlqsbyyn000165x6j5zxcm0z. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/grimoire.csv (player column values are display names; source player = grimoire)
 * CZT user ID: cmlqsbyyn000165x6j5zxcm0z
 *
 * Usage:
 *   pnpm db:import-user-grimoire              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-grimoire.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmlqsbyyn000165x6j5zxcm0z';
const SOURCE_PLAYER_ID = 'grimoire';
const CSV_PATH = path.join(root, 'top-178-csv', 'grimoire.csv');
...
async function main() {
  ...
  console.log('--- grimoire full transfer ---');
  ...
  console.log('\n--- Done. grimoire transfer complete. ---');
}
