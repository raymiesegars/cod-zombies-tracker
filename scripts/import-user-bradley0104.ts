#!/usr/bin/env npx tsx
/**
 * Full transfer for Bradley0104: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmlqtmbgx00059uzbkloqjiyi. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/Bradley0104.csv (player column values are display names; source player = Bradley0104)
 * CZT user ID: cmlqtmbgx00059uzbkloqjiyi
 *
 * Usage:
 *   pnpm db:import-user-bradley0104              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-bradley0104.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmlqtmbgx00059uzbkloqjiyi';
const SOURCE_PLAYER_ID = 'Bradley0104';
const CSV_PATH = path.join(root, 'top-178-csv', 'Bradley0104.csv');
...
async function main() {
  ...
  console.log('--- Bradley0104 full transfer ---');
  ...
  console.log('\n--- Done. Bradley0104 transfer complete. ---');
}
