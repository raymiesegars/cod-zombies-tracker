/**
 * Safe deployment for Advanced Warfare Exo Zombies.
 * Single script: migrate deploy + add-aw content.
 *
 ...
 *
 * Run: pnpm run db:deploy-aw              # Local/dev DB (default)
 * Run: pnpm run db:deploy-aw -- --production   # Production DB
 */
...
console.log('=== Advanced Warfare Exo Zombies Deployment (safe, additive only) ===\n');
...
// Step 2: Migrate deploy (applies pending migrations)
...
// Step 3: Add AW content (creates only; skips existing)
console.log('\nStep 3: Adding AW content (game, maps, challenges, easter eggs, achievements)...');
const addEnv = { ...process.env, ...(useProduction ? { DEPLOY_AW_PRODUCTION: '1' } : {}) };
const addResult = spawnSync('pnpm', ['run', 'db:add-aw'], {
  stdio: 'inherit',
  env: addEnv,
  cwd: root,
});
if (addResult.status !== 0) {
  console.error('Add AW content failed.');
  process.exit(1);
}

console.log('\n=== AW deployment complete. ===');
