#!/usr/bin/env npx tsx
/**
 * Full transfer for SM0k3Y: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmlyp3llo000020vwc0ucyu88. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/SM0k3Y.csv (player column values are display names; source player = SM0k3Y)
 * CZT user ID: cmlyp3llo000020vwc0ucyu88
 *
 * Usage:
 *   pnpm exec tsx scripts/import-user-sm0k3y.ts              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-sm0k3y.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmlyp3llo000020vwc0ucyu88';
const SOURCE_PLAYER_ID = 'SM0k3Y';
const CSV_PATH = path.join(root, 'top-178-csv', 'SM0k3Y.csv');
...
async function main() {
  ...
  console.log('--- SM0k3Y full transfer ---');
