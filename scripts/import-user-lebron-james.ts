#!/usr/bin/env npx tsx
/**
 * Full transfer for LeBron James: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmlvocpbj0006ar6ml9vz7hsm. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/LeBron James.csv (player column values are display names; source player = LeBron James)
 * CZT user ID: cmlvocpbj0006ar6ml9vz7hsm
 *
 * Usage:
 *   pnpm db:import-user-lebron-james              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-lebron-james.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmlvocpbj0006ar6ml9vz7hsm';
const SOURCE_PLAYER_ID = 'LeBron James';
const CSV_PATH = path.join(root, 'top-178-csv', 'LeBron James.csv');
...
async function main() {
  ...
  console.log('--- LeBron James full transfer ---');
  ...
  console.log('\n--- Done. LeBron James transfer complete. ---');
}
