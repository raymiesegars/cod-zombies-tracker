#!/usr/bin/env npx tsx
/**
 * Full transfer for WhiteBoyPl4ys: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmlrfnm6k000012f2xz6ufq75. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/WhiteBoyPl4ys.csv (player column values are display names; source player = WhiteBoyPl4ys)
 * CZT user ID: cmlrfnm6k000012f2xz6ufq75
 *
 * Usage:
 *   pnpm db:import-user-whiteboypl4ys              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-whiteboypl4ys.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmlrfnm6k000012f2xz6ufq75';
const SOURCE_PLAYER_ID = 'WhiteBoyPl4ys';
const CSV_PATH = path.join(root, 'top-178-csv', 'WhiteBoyPl4ys.csv');
...
async function main() {
  ...
  console.log('--- WhiteBoyPl4ys full transfer ---');
  ...
  console.log('\n--- Done. WhiteBoyPl4ys transfer complete. ---');
}
