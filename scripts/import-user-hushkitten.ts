#!/usr/bin/env npx tsx
/**
 * Full transfer for Hushkitten: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmmkkpirp0004l5fz0f6fd4xr. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/Hushkitten.csv (player column values are display names; source player = Hushkitten)
 * CZT user ID: cmmkkpirp0004l5fz0f6fd4xr
 *
 * Usage:
 *   pnpm db:import-user-hushkitten              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-hushkitten.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmmkkpirp0004l5fz0f6fd4xr';
const SOURCE_PLAYER_ID = 'Hushkitten';
const CSV_PATH = path.join(root, 'top-178-csv', 'Hushkitten.csv');
...
async function main() {
  ...
  console.log('--- Hushkitten full transfer ---');
  ...
  console.log('\n--- Done. Hushkitten transfer complete. ---');
}
