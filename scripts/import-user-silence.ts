#!/usr/bin/env npx tsx
/**
 * Full transfer for silence: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmmdyvrwa000112xe1llrhnmj. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/silence.csv (player column values are display names; source player = silence)
 * CZT user ID: cmmdyvrwa000112xe1llrhnmj
 *
 * Usage:
 *   pnpm db:import-user-silence              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-silence.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmmdyvrwa000112xe1llrhnmj';
const SOURCE_PLAYER_ID = 'silence';
const CSV_PATH = path.join(root, 'top-178-csv', 'silence.csv');
...
async function main() {
  ...
  console.log('--- silence full transfer ---');
  ...
  console.log('\n--- Done. silence transfer complete. ---');
}
