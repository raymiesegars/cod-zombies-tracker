#!/usr/bin/env npx tsx
/**
 * Full transfer for Phobos: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmmdqzv9i0000114dhvjtj6ny. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/Phobos.csv (player column values are display names; source player = Phobos)
 * CZT user ID: cmmdqzv9i0000114dhvjtj6ny
 *
 * Usage:
 *   pnpm db:import-user-phobos              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-phobos.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmmdqzv9i0000114dhvjtj6ny';
const SOURCE_PLAYER_ID = 'Phobos';
const CSV_PATH = path.join(root, 'top-178-csv', 'Phobos.csv');
...
async function main() {
  ...
  console.log('--- Phobos full transfer ---');
