#!/usr/bin/env npx tsx
/**
 * Full transfer for Gardningtools: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmlvf1ikw000097igho2zsb3d. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/Gardningtools.csv (player column values are display names; source player = Gardningtools)
 * CZT user ID: cmlvf1ikw000097igho2zsb3d
 *
 * Usage:
 *   pnpm db:import-user-gardningtools              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-gardningtools.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmlvf1ikw000097igho2zsb3d';
const SOURCE_PLAYER_ID = 'Gardningtools';
const CSV_PATH = path.join(root, 'top-178-csv', 'Gardningtools.csv');
...
async function main() {
  ...
  console.log('--- Gardningtools full transfer ---');
  ...
  console.log('\n--- Done. Gardningtools transfer complete. ---');
}
