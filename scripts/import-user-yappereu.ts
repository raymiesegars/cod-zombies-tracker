#!/usr/bin/env npx tsx
/**
 * Full transfer for YapperEU: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmmdsenoj0000ggpql3txjvz4. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/YapperEU.csv (player column values are display names; source player = YapperEU)
 * CZT user ID: cmmdsenoj0000ggpql3txjvz4
 *
 * Usage:
 *   pnpm db:import-user-yappereu              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-yappereu.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmmdsenoj0000ggpql3txjvz4';
const SOURCE_PLAYER_ID = 'YapperEU';
const CSV_PATH = path.join(root, 'top-178-csv', 'YapperEU.csv');
...
async function main() {
  ...
  console.log('--- YapperEU full transfer ---');
  ...
  console.log('\n--- Done. YapperEU transfer complete. ---');
}
