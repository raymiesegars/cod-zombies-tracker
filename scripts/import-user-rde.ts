#!/usr/bin/env npx tsx
/**
 * Full transfer for rde: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmm8kxhkv0000142wfjoz8hxf. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/rde.csv (player column values are display names; source player = rde)
 * CZT user ID: cmm8kxhkv0000142wfjoz8hxf
 *
 * Usage:
 *   pnpm db:import-user-rde              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-rde.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmm8kxhkv0000142wfjoz8hxf';
const SOURCE_PLAYER_ID = 'rde';
const CSV_PATH = path.join(root, 'top-178-csv', 'rde.csv');
