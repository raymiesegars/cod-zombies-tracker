#!/usr/bin/env npx tsx
/**
 * Full transfer for THAT GUY: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmlqtjqng000cu994seq1kii3. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/THAT GUY.csv (player column values are display names; source player = THAT GUY)
 * CZT user ID: cmlqtjqng000cu994seq1kii3
 *
 * Usage:
 *   pnpm db:import-user-that-guy              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-that-guy.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmlqtjqng000cu994seq1kii3';
const SOURCE_PLAYER_ID = 'THAT GUY';
const CSV_PATH = path.join(root, 'top-178-csv', 'THAT GUY.csv');
...
async function main() {
  ...
  console.log('--- THAT GUY full transfer ---');
  ...
  console.log('\n--- Done. THAT GUY transfer complete. ---');
}
