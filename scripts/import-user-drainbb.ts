#!/usr/bin/env npx tsx
/**
 * Full transfer for drainbb: import CSV → reunlock achievements → recompute verified XP.
 * Safe: only affects the single CZT user cmlr2i8cu0000kzag9nem5br8. No truncation, no
 * deletion of other users' data.
 *
 * CSV: top-178-csv/drainbb.csv (player column values are display names; source player = drainbb)
 * CZT user ID: cmlr2i8cu0000kzag9nem5br8
 *
 * Usage:
 *   pnpm db:import-user-drainbb              # Run all 3 steps
 *   pnpm exec tsx scripts/import-user-drainbb.ts --dry-run   # Import dry-run only (no DB writes)
 */
...
const CZT_USER_ID = 'cmlr2i8cu0000kzag9nem5br8';
const SOURCE_PLAYER_ID = 'drainbb';
const CSV_PATH = path.join(root, 'top-178-csv', 'drainbb.csv');
...
async function main() {
  ...
  console.log('--- drainbb full transfer ---');
  ...
  console.log('\n--- Done. drainbb transfer complete. ---');
}
