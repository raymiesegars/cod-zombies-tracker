/**
 * BOCW Balance Patch Migration
 *
 * SAFE: Only affects BOCW game maps. No truncation, no deletion of user logs.
 * - Deactivates challenges that don't exist per map (isActive=false)
 * - Creates new challenges (PURIST, NO_ARMOR, NO_JUG, EXFIL_SPEEDRUN, BUILD_EE_SPEEDRUN, etc.)
 * - Replaces BOCW achievements with WR-based definitions; recalculates user XP
 *
 * Run AFTER schema migration (20260223000000_bocw_rebalance).
 *
 * Usage:
 *   pnpm exec tsx scripts/bocw-balance-patch.ts           # Run against .env.local (dev)
 *   pnpm exec tsx scripts/bocw-balance-patch.ts --dry-run  # Preview without writing
 */
...
import { BOCW_MAP_CONFIG, getBocwChallengeTypeLabel, type BocwMapSlug } from '../src/lib/bocw/bocw-map-config';
import type { ChallengeType } from '@prisma/client';

const DRY_RUN = process.argv.includes('--dry-run');

const BOCW_SPEEDRUN_TYPES: { type: ChallengeType; name: string }[] = [
  { type: 'ROUND_10_SPEEDRUN', name: 'Round 10 Speedrun' },
  { type: 'ROUND_20_SPEEDRUN', name: 'Round 20 Speedrun' },
  ...
  { type: 'ROUND_935_SPEEDRUN', name: 'Round 935 Speedrun' },
...
async function main() {
  ...
  const game = await prisma.game.findFirst({ where: { shortName: 'BOCW' } });
  if (!game) {
    console.log('BOCW game not found. Exiting.');
    return;
  }
  ...
  if (maps.length === 0) {
    console.log('No BOCW maps found. Exiting.');
    return;
  }
  ...
  for (const map of maps) {
    const cfg = BOCW_MAP_CONFIG[map.slug as BocwMapSlug];
    if (!cfg) continue;

    const allowedTypes = new Set(cfg.challengeTypes);
    const speedrunTypeSet = new Set(BOCW_SPEEDRUN_TYPES.map((s) => s.type));
    ...
    for (const t of Array.from(allowedTypes)) {
      ...
      const name = getBocwChallengeTypeLabel(t) || t.replace(/_/g, ' ');
      if (!DRY_RUN) {
    ...
    const speedrunToCreate = BOCW_SPEEDRUN_TYPES.filter(({ type }) => {
      if (type === 'BUILD_EE_SPEEDRUN' && !cfg.speedrunWRs?.buildEeSpeedrunWR) return false;
      if (type === 'ROUND_10_SPEEDRUN' && !cfg.speedrunWRs?.r10) return false;
      if (type === 'ROUND_20_SPEEDRUN' && !cfg.speedrunWRs?.r20) return false;
      if (type === 'ROUND_935_SPEEDRUN' && !cfg.speedrunWRs?.r935) return false;
  ...
  console.log('2. Updating BOCW achievements...');
  ...
  for (const map of mapsWithChallenges) {
    const mapDefs = getMapAchievementDefinitions(map.slug, 0, map.game?.shortName ?? 'BOCW');
    const speedrunDefs = getSpeedrunAchievementDefinitions(map.slug, map.game?.shortName ?? 'BOCW');
  ...
  if (DRY_RUN) {
    ...
    console.log('\nBOCW balance patch applied successfully.');
  }
